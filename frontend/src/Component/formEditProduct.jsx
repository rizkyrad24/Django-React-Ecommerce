...
const FormEditProduct = ({ user, data }) => {
    ...
    const editProduct = async () => {
        ...
        try {
            await axios.put(`http://localhost:8000/product/mylist/action/${data.id}/`, body, config);
                ...
                <form className="mb-3 row g-3" onSubmit={e => handlingSubmit(e)}>
                    ...
                    <div className="col-4">
                        ...
                        <select onChange={e => handlingInput(e)} name="category" className="form-select" id="category">
                            ...
                            data.id === category ? <option selected key={data.id} value={data.id}>{data.category}</option> : <option key={data.id} value={data.id}>{data.category}</option>
                            ) : <></> }
                        </select>
                    </div>
                    ...
                    <div className="col-12">
                        ...
                        <select onChange={e => handlingInput(e)} name="address" className="form-select" id="address">
                            ...
                            data.id === address ? <option selected key={data.id} value={data.id}>{data.address_name}, {data.detail_address}, {data.area}, {data.suburb}, {data.city}, {data.state}</option> : <option key={data.id} value={data.id}>{data.address_name}, {data.detail_address}, {data.area}, {data.suburb}, {data.city}, {data.state}</option> 
                            ) : <></> }
                        </select>
                    ...
                    <div className="col-12">
                        <div className="form-check">
                            { is_active ? (
                                <input checked name="is_active" value="true" onClick={e => handlingCheckbox(e)} className="form-check-input" type="checkbox" id="is_active"/>
                            ) : (
