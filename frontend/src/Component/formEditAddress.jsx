...
const FormEditAddress = ({ user, data }) => {
    const [ formData, setFormData ] = useState({
        ...
        longlitude: parseFloat(data.longlitude)
    });
    ...
    const editAddress = async (user) => {
        ...
        try {
            await axios.put(`http://localhost:8000/product/shipping/address/action/${data.id}/`, body, config);
    ...
    // function for initial value form
    const getStateAPIInitial = async () => {
        const config = {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('access')}`
        ...
        try {
            const res = await axios.get(`http://localhost:8000/product/address/state/${data.country_id}/`, config);
            setStateForm(res.data);
        } catch (error) {
            console.log(error);
        };
    };
    const getCityAPIInitial = async () => {
        const config = {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('access')}`
        ...
        try {
            const res = await axios.get(`http://localhost:8000/product/address/city/${data.state_id}/`, config);
            setCityForm(res.data);
        } catch (error) {
            console.log(error);
        };
    };
    const getSuburbAPIInitial = async () => {
        const config = {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('access')}`
        ...
        try {
            const res = await axios.get(`http://localhost:8000/product/address/suburb/${data.city_id}/`, config);
            setSuburbForm(res.data);
        } catch (error) {
            console.log(error);
        };
    };
    const getAreaAPIInitial = async () => {
        const config = {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('access')}`
        ...
        try {
            const res = await axios.get(`http://localhost:8000/product/address/area/${data.suburb_id}/`, config);
            setAreaForm(res.data);
        } catch (error) {
            console.log(error);
        };
                ...
                <form className="mb-3 row g-3" onSubmit={e => handlingSubmit(e)}>
                    ...
                    <div className="col-4">
                        ...
                        <select onChange={(e) => {getStateAPI(); getValueInput(e)}} name="country" className="form-select" id="country_id">
                            ...
                            { countryForm ? countryForm.map( data => (
                            data.id === country_id ? <option selected key={data.id} value={data.id}>{data.name}</option> : <option key={data.id} value={data.id}>{data.name}</option>
                            ) ) : <></> }
                        </select>
                    </div>
                    <div className="col-4">
                        ...
                        <select onChange={(e) => {getCityAPI(); getValueInput(e)}} name="state" className="form-select" id="state_id">
                            <option defaultValue>__address__</option>
                            { stateForm ? stateForm.map( data => (
                            data.id === state_id ? <option selected key={data.id} value={data.id}>{data.name}</option> : <option key={data.id} value={data.id}>{data.name}</option>
                            ) ) : <></> }
                        </select>
                    </div>
                    <div className="col-4">
                        ...
                        <select onChange={(e) => {getSuburbAPI(); getValueInput(e)}} name="city" className="form-select" id="city_id">
                            <option defaultValue>__address__</option>
                            { cityForm ? cityForm.map( data => (
                            data.id === city_id ? <option selected key={data.id} value={data.id}>{data.name}</option> : <option key={data.id} value={data.id}>{data.name}</option>
                            ) ) : <></> }
                        </select>
                    </div>
                    <div className="col-6">
                        ...
                        <select onChange={(e) => {getAreaAPI(); getValueInput(e)}} name="suburb" className="form-select" id="suburb_id">
                            <option defaultValue>__address__</option>
                            { suburbForm ? suburbForm.map( data => (
                            data.id === suburb_id ? <option selected key={data.id} value={data.id}>{data.name}</option> : <option key={data.id} value={data.id}>{data.name}</option>
                            ) ) : <></> }
                        </select>
                    </div>
                    <div className="col-6">
                        ...
                        <select onChange={(e) => {getValueInput(e)}} name="area" className="form-select" id="area_id">
                            <option defaultValue>__address__</option>
                            { areaForm ? areaForm.map( data => (
                            data.id === area_id ? <option selected key={data.id} value={data.id} data-lat={data.lat} data-lng={data.lng}>{data.name}</option> : <option key={data.id} value={data.id} data-lat={data.lat} data-lng={data.lng}>{data.name}</option>
                            ) ) : <></> }
                        </select>
                    </div>
                    <div className="col-12">
                    ...
                    <div className="col-12">
                        ...
                        <button onClick={() => {showMap(); getAreaLocation()}} name="location" type="button" className="btn btn-primary" id="location">Pin Point</button>
                    </div>
                    <div className="col-12 d-grid gap-2">
                        <button className="btn btn-primary" type="submit">Edit Address</button>
                    </div>
                </form>
            </div>
